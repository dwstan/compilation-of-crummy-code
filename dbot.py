...
prefixes_file = "/home/pi/discordbot/json_storage/prefixes.json"
guild_settings_file = "/home/pi/discordbot/json_storage/guild_settings.json"
work_reminder_file = "/home/pi/discordbot/json_storage/work_reminder.json"
work_stats_file = "/home/pi/discordbot/json_storage/work_stats.json"
egg_dict_file = "/home/pi/discordbot/json_storage/egg_dict.json"
frame_info_file = "/home/pi/discordbot/json_storage/frame_info.json"
bits_file = "/home/pi/discordbot/json_storage/bits.json"

# koibito json files
_question_storage_file = "/home/pi/discordbot/char_questions/base_files/_question_storage.json"
_similar_questions_file = "/home/pi/discordbot/char_questions/base_files/_similar_questions.json"
koibito_aliases_file = "/home/pi/discordbot/char_questions/base_files/koibito_aliases.json"

_template_char_file = "/home/pi/discordbot/char_questions/_template_char.json"
...
# ==============================================================================
    ...
    # on ready, go to home page and extract highest sauce number
def find_latest_sauce():
    home_web = requests.get('https://nhentai.net')
    home_html = home_web.text
    
    index_new_uploads = home_html.find(' New Uploads')
    home_html = home_html[index_new_uploads: index_new_uploads+300]
    
    index_recent_digits = home_html.find('/g/')
    home_html = home_html[index_recent_digits+3: index_recent_digits+3+6]
    return home_html

    # sends this through to the sauce generator
def update_sauce(latest_sauce):
    # int cast, turn string into int
    global max_digits
    if max_digits != int(latest_sauce):
        max_digits = int(latest_sauce)
        print(f'> Max Sauce is now: {max_digits}')
...
emote_array = {
        ...
        'moose': '<:moose:733077344352337991>',
        'mooseball': '<:mooseball:733836035250716763>',
        'moosecry': '<:moosecry:733884432338911284>',
        'bwide': '<:brandonWideHead:715049362732089364>',
        'lappdumb': '<:lappdumb:733484533223260231>',
        'texdumb': '<:texdumb:733844480427557005>',
        'exuhyper': '<:exuhyper:736415338773741659>',
        'skadi': '<:skadi:734140756033273938>',
        'orundum': '<:orundum:733833636163616848>',
        'lmd': '<:lmd:733833646380810372>',
        'originite': '<:originite:733833681294065755>',
        'credits': '<:credits:733833693101162568>',
}
...
sauce_emojis = [
    # 0
    [':slight_smile:', ':smirk:', ':smile:'],
    # 1
    [':flushed:',':face_with_raised_eyebrow:',':no_mouth:'],
    # 2
    [':thinking:',':face_with_monocle:'],
    # 3
    [':sob:',':pensive:',':cry:',':pleading_face:'],
    # 4
    [':fearful:',':scream:',':skull:'],
    # 5
    [':nauseated_face:',':face_vomiting:'],
    # 6
    [':heart_eyes:',':smiling_face_with_3_hearts:',':kissing_heart:',':stuck_out_tongue:',':smirk:',':innocent:',':stuck_out_tongue:']
]

sauce_responses = [
    # 0. happy
    [
        'Here is your sauce sir.',
        'Bon appetit. ',
        'Enjoy your meal.',
        'Your nuclear launch code: ',
        'The regular for you?',
        'Just the classic cup of tea.',
        'Here\'s a fresh one.',
        'A pinch of sugar.'
    ],
    ...
    # 1. startled
    [
        'Uhh, here\'s your sauce.',
        'Wait a second...',
        'Hmm, this one\'s a bit off.',
        'Well then.',
        'I believe this is your kink.'
    ],
    ...
    [
        '???',
        'Interesting.',
        'Incorrect sauce number acquired.',
        'Upon closer inspection, this one is quite abnormal.',
    ...
    [
        ...
        'Very emotional piece indeed.',
        'This will make you question your life choices.',
        'Do mind, this might be a little overwhelming.',
        'This isn\'t right...'
    ],

    # 4. feared
    [
        'Be warned, this one holds great danger...',
        'Be wary around this part of town...',
        'This one has an immensely disturbing aura...',
        'Turn back now...',
        'Perhaps this one isn\'t for our eyes to see...'
    ],

    # 5. gross
    [
        'This one is best left unchecked.',
        'There\'s a bad rating for a reason.',
        'Disgusting.',
        'What a vile creation.',
        'This one took a turn for the worst...'
    ],
    ...
    [
        ...
        'What a lovely find.',
        'You rolled, we delivered.',
        'Kinky, I\'m assured.',
        'This one is a godsend.',
        'Cute vibes ahead.',
        'I found one to your liking.'
    ]
]
...
sauce_responses_length = [
    len(sauce_responses[0]),
    len(sauce_responses[1]),
    len(sauce_responses[2]),
    len(sauce_responses[3]),
    len(sauce_responses[4]),
    len(sauce_responses[5]),
    len(sauce_responses[6])
]



def parseTags(hexcode):
    url = str(f'https://www.nhentai.net/g/{hexcode}')
    r = requests.get(url)
    respData = r.text

    respData = respData[str(respData).index('<meta name="twitter:desc') - 24:-(len(respData) - 1100)]
    tags = re.findall(r'<meta name="twitter:description" content="(.*?)" />', str(respData))

    return tags

def parseWebsite(website):
    url = str(f'{website}')
    r = requests.get(url)
    return r.text

@client.command()
async def parse(ctx, *, hexcode):
    if await isCorrectChannel(ctx, 'parse'):
        if await isWhitelisted(ctx, admin_whitelist, perm = 'use the parse command'):
            msgProgress = await ctx.send(f'Command for parse in progress...')
            tags = parseTags(hexcode)
            
            # editing a message
            await msgProgress.edit(content = f'Parsing successful for `https://www.nhentai.net/g/{hexcode}`.\n:warning: Tags Found! :warning:\n`{tags}`')
            #await ctx.send(f'Parsing successful for `https://www.nhentai.net/g/{hexcode}`.\n:warning: Tags Found! :warning:\n`{tags}`')
        else:
            await ctx.send(f':no_entry: Only a `{admin_whitelist[0]}` can use the `<parse>` command.')



sauce_tags = [
    # 0 blessed
    ['miko', 'kimono', 'chinese dress', 'shimapan', 'sundress', 'bunny girl'],
    # 1 immunity
    ['yuri','no penetration', 'catgirl'],
    # 2 gay
    ['yaoi', 'tomgirl', 'dickgirl on male'], 
    # 3 cursed
    ['lolicon', 'scat', 'guro', 'vore', 'stomach deformation', 'impregnation', 'netorare', 'incest', 'bestiality', 'amputee'],
    # 4 abomination
    ['horse', 'all the way through', 'unusual teeth', 'eggs', 'insect', 'cannibalism']
]
...
async def sauce_generator(ctx, num):
    generated_sauce = 0
    if num != None:
        generated_sauce = num
    else:
        generated_sauce = random.randint(1, max_digits)

    index = random.randint(0, len(sauce_responses)-1 )
    
    response_index = random.randint(0, sauce_responses_length[index]-1 )
    emoji_index = random.randint(0, sauce_emojis_length[index] -1)

    the_string = f'{sauce_emojis[index][emoji_index]} {sauce_responses[index][response_index]}\nhttps://www.nhentai.net/g/{generated_sauce}'
    msg = await ctx.send(the_string)
    
    await sauce_editor(ctx, generated_sauce, msg)


async def sauce_editor(ctx, generated_sauce, msg): 
    r = requests.get(f'https://www.nhentai.net/g/{generated_sauce}')
    if not '<title>404 - Not Found' in r.text[250:310]:
        # finding tags by putting substring
        respData = r.text[str(r.text).index('<meta name="twitter:desc') - 24:-(len(r.text) - 1100)]
        # tags = all of your sauce's tags in a string
        tags = str( re.findall(r'<meta name="twitter:description" content="(.*?)" />', str(respData)) )
        #for i in range(0, len(sauce_tags) ):
        ...
        # go through all sauce tags, add them to array tag_stor
        for i in range(len(sauce_tags)):
            for tag in sauce_tags[i]:
                if tags.find(tag) != -1:
                    if len(tag_stor[i]) == 0:
                        tag_stor[i] = f'{category_emotes[i]} `Rolled {sauce_tags_category[i]}`: {tag}'
                    else:
                        tag_stor[i] += f', {tag}'
                    
        # convert array tagStor to string
        all_tags = ""
        for i in range(len(tag_stor)):
            if len(tag_stor[i]) > 0:
                all_tags += tag_stor[i] + "\n"
        
        # if the person got any tags
        if len(all_tags) > 0:
            # add all_tags to end of message
            await msg.edit(content=f'{msg.content}\n{all_tags}')
    else:
        await msg.edit(content=f':no_entry: 404 found.\nhttps://www.nhentai.net/g/{generated_sauce}')


@client.command(aliases=['s'])
async def sauce(ctx, arg1=None, arg2=None):
    if await isCorrectChannel(ctx, 'sauce') == True:
        if arg1 == 'update':
            latest_sauce = find_latest_sauce()
            update_sauce(latest_sauce)
            await ctx.send(f':white_check_mark: Latest sauce: {latest_sauce}.')
            await client.change_presence(activity=discord.Game(f'Ara Ara~ {latest_sauce}'))
        elif arg1 == 'new':
            if arg2 != None and str(arg2).isdigit():
                arg2 = int(arg2)
                if arg2 <= 100:
                    await sauce_generator(ctx, max_digits - arg2)
                else:
                    await ctx.message.delete()
                    await ctx.send(f':no_entry Recent Sauce subtraction may not exceed 100.', delete_after=5)
            else:
                await sauce_generator(ctx, max_digits)
        else:
            await sauce_generator(ctx, arg1)


# ==============================================================================
# work reminder section
...
#         recovery_date = datetime(recovery_date[0], recovery_date[1], recovery_date[2], recovery_date[3], recovery_date[4])
...
#             frame_print += f"[{the_frame['frame']}] : {the_frame['bits'][0]}, {the_frame['bits'][1]} \n"
    ...
    if len(args) >= 3:
        for i in range(0, len(args)):
            if args[i].isdigit():
                ...
                bits[1] = args[i]
    fname = fname.strip(" ")
    print(f"received FRAME NAME: '{fname}'")
...
async def frameadd(ctx, *, args=""):
    if await isWhitelisted(ctx, commander_in_chief, "add a frame to the `frame_info.json` file."):
        ...
        if len(fname) > 0 and bits[0] in all_bits['bits'] and bits[1] in all_bits['bits']:
            ...
            if index != None:
                # minus 2 to index, match with frame shop
...
def highlight_koibito_keywords(simq_question):
    simq_question.replace("least", "**LEAST**")
    simq_question.replace("most", "**MOST**")
...
@client.event # gyazo embed remover
async def on_message(message):
    #print(message.author.name + " : " + message.content)
    #await gyazoCheck(message)
    ...
    # jason print screen
    # await json_prnt_sc(message)
    await client.process_commands(message)

async def json_prnt_sc(message):
    #if jason sending a message:
    if (message.author.id == 152613011670958090 or message.author.id == 263853089419821056) and message.content.find("prnt.sc/") != -1: # if puf
        print(f'> PRNT_SC: message from: {message.author}')
        time.sleep(1)
        # https://prnt.sc/116oay9
        print(f'> PRNT_SC: does message have embed? {len(message.embeds) > 0}')
        if len(message.embeds) > 0:
            embed_dict = message.embeds[0].to_dict()
            await message.delete()
            await message.channel.send(f"{embed_dict['thumbnail']['url']}")
...
client.run('NzMxNjQ0NTgyMTY0NDMwODcw.XwpEwg.lJttY7xnYqo1I0j3mCetZy47tkw')